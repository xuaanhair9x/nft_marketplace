...
const MintNftRentable = ({ nftRentable }) => {
  ...
  const createNFT = async () => {
    ...
    try {
      ...
      fetch('http://127.0.0.1:3333/write/', requestOptions)
        .then(response => response.json())
        .then(data => mintThenList(data.path))
        .catch(err => {
  ...
  const mintThenList = async (uri) => {
    // mint nft 
    await(await nftRentable.mint(uri)).wait()
  }
...
export default MintNftRentable